...
```{r save fitDLM output, message=FALSE,  warning=FALSE, results="hide"}

# save output of fitDLM, to start making boxplot function

saveRDS(dlm_model, file = paste(here(), "simDataIncreaseProdyCU1.Rdata", sep="/"), version=3)
...
for (i in 1:4){
 genericRecoverySim(simPar=simPar[i,], cuPar=cuPar,  srDat=srDat,
            variableCU=FALSE, ricPars=NULL, #ricPars,  #cuCustomCorrMat = cuCustomCorrMat,
            dirName="example", nTrials=5, makeSubDirs=FALSE, random=FALSE)}

```

Borrow Brendan's example code to put all results in the same dataframe.
...
for(i in 1:4){
  simData[[i]] <- simData[[i]] %>% 
    dplyr::filter(CU == 1) %>% 
    mutate(scenario = scenNames[i]) %>% 
    select(scenario, everything()) %>% 
    rename(byr=year, spwn=spawners, rec=recruits, alpha_true=alpha, beta_true=beta)
    
  simData[[i]] <- cbind(simData[[i]],as.data.frame(matrix(NA,nrow(simData[[i]]),4))) %>%
    rename(alpha=V1, beta=V2, alpha_se=V3, beta_se=V4)
}

iteration <- unique(simData[[i]]$iteration)

#up to here:
# look at doing this with purrr or furrr later
